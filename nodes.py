...
# ---------------------------------------------------------------------------
# Node: Export Unity Humanoid bundle
# ---------------------------------------------------------------------------
class Kimodo_ExportUnity:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "motion": ("KIMODO_MOTION",),
                "character_fbx_path": ("STRING", {
                    "default": "",
                    "tooltip": "Mixamo-rigged FBX character. Absolute or relative to ComfyUI/input.",
                }),
                "filename_prefix": ("STRING", {"default": "kimodo_unity"}),
                "sample_index": ("INT", {"default": 0, "min": 0, "max": 15}),
            },
            "optional": {
                "yaw_offset": ("FLOAT", {"default": 0.0, "min": -180.0, "max": 180.0, "step": 1.0}),
                "scale": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 10.0, "step": 0.01}),
            },
        }

    RETURN_TYPES = ("STRING", "STRING")
    RETURN_NAMES = ("unity_zip", "fbx_path")
    FUNCTION = "export"
    CATEGORY = "Kimodo/Export"
    OUTPUT_NODE = True

    def export(self, motion, character_fbx_path, filename_prefix="kimodo_unity",
               sample_index=0, yaw_offset=0.0, scale=0.0):
        from platform_exports import make_unity_bundle

        fbx_path = Kimodo_ExportFBX().export(
            motion, character_fbx_path, filename_prefix + "_Humanoid",
            sample_index, yaw_offset, scale,
        )[0]
        if not fbx_path or not os.path.isfile(fbx_path):
            return (f"Unity export failed: {fbx_path}", "")
        output = motion.output_dict
        frame_count = int(output["posed_joints"].shape[1])
        zip_path, fbx_path = make_unity_bundle(
            fbx_path, folder_paths.get_output_directory(), filename_prefix,
            " ".join(motion.texts or []), motion.fps, frame_count,
        )
        print(f"[Kimodo] Unity Humanoid bundle: {zip_path}", flush=True)
        return (zip_path, fbx_path)


# ---------------------------------------------------------------------------
# Node: Export Rive runtime bundle
# ---------------------------------------------------------------------------
class Kimodo_ExportRive:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "motion": ("KIMODO_MOTION",),
                "filename_prefix": ("STRING", {"default": "kimodo_rive"}),
                "sample_index": ("INT", {"default": 0, "min": 0, "max": 15}),
                "view": (["front", "side"], {"default": "front"}),
                "pixels_per_meter": ("FLOAT", {"default": 240.0, "min": 1.0, "max": 2000.0, "step": 1.0}),
                "include_root_motion": ("BOOLEAN", {"default": True}),
            },
            "optional": {
                "rive_template_path": ("STRING", {
                    "default": "",
                    "tooltip": "Optional authored .riv skin; absolute or relative to ComfyUI/input.",
                }),
            },
        }

    RETURN_TYPES = ("STRING", "STRING", "STRING")
    RETURN_NAMES = ("rive_zip", "motion_json", "preview_gif")
    FUNCTION = "export"
    CATEGORY = "Kimodo/Export"
    OUTPUT_NODE = True

    def export(self, motion, filename_prefix="kimodo_rive", sample_index=0,
               view="front", pixels_per_meter=240.0, include_root_motion=True,
               rive_template_path=""):
        ...
        result = make_rive_bundle(
            ...
            rive_template_path, comfy_root,
        )
        print(f"[Kimodo] Rive runtime bundle: {result[0]}", flush=True)
        return result


class Kimodo_RenderRiveSkin:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
            "motion": ("KIMODO_MOTION",),
            ...
            "height": ("INT", {"default": 720, "min": 128, "max": 2048, "step": 8}),
        }}

    RETURN_TYPES = ("STRING", "STRING", "STRING")
    RETURN_NAMES = ("video_mp4", "preview_gif", "asset_zip")
    FUNCTION = "render"
    CATEGORY = "Kimodo/Export"
    OUTPUT_NODE = True
    ...
    def render(self, motion, rive_skin_path, filename_prefix="kimodo_rive_character",
        ...
        return render_rive_skin_video(
            motion, folder_paths.get_output_directory(), filename_prefix,
            sample_index, view, pixels_per_meter, include_root_motion,
            rive_skin_path, bone_mapping_json, root, width, height)
...
NODE_CLASS_MAPPINGS = {
    ...
    "Kimodo_ExportUnity": Kimodo_ExportUnity,
    "Kimodo_ExportRive": Kimodo_ExportRive,
    "Kimodo_RenderRiveSkin": Kimodo_RenderRiveSkin,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "Kimodo_ExportUnity": "Kimodo Export Unity Humanoid Bundle",
    "Kimodo_ExportRive": "Kimodo Export Rive Runtime Bundle",
    "Kimodo_RenderRiveSkin": "Kimodo Render Rive Skinned Character",
}
